...
class CustomerCreate(BaseModel):
    ...
    @field_validator("name", "contact_person", "city", "state", "country", "tax_id", mode="before")
    @classmethod
    def strip_text(cls, value: str | None) -> str | None:
        return value.strip() if isinstance(value, str) else value

    @model_validator(mode="after")
    def require_customer_name(self) -> "CustomerCreate":
        if not self.name and not self.customer_name:
            raise ValueError("name or customer_name is required")
        return self
